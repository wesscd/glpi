...
setup_apache_security() {
    ...
    a2enmod headers
    check_execution "Módulo headers habilitado." "Falha ao habilitar módulo headers."
...
EOF
    ...
    check_execution "Configuração security-headers habilitada." "Falha ao habilitar configuração security-headers."
...
# Função para configurar NTP CLIENT
install_ntp_client() {
    echo "[INFO] Instalando cliente NTP..."

    # Atualiza lista de pacotes
    sudo apt update -y

    # Instala o pacote ntpdate (caso ainda não esteja instalado)
    sudo apt install -y ntpdate

    # Sincroniza imediatamente com o servidor NTP do Brasil
    echo "[INFO] Sincronizando data e hora com pool.ntp.br..."
    sudo ntpdate pool.ntp.br

    # Ajusta o fuso horário para o do sistema (opcional: pode trocar para America/Sao_Paulo)
    echo "[INFO] Garantindo fuso horário correto..."
    sudo timedatectl set-timezone America/Sao_Paulo

    echo "[INFO] Data e hora atualizadas:"
    date
}
